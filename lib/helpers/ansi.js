/**
 * Check if text contains ANSI codes
 * @param {string} txt
 * @returns {boolean}
 */
var hasAnsi = function (txt) {
    if (txt === void 0) { txt = ''; }
    if (!txt.length) {
        return false;
    }
    else {
        var pattern = [
            '[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:[a-zA-Z\\d]*(?:;[a-zA-Z\\d]*)*)?\\u0007)',
            '(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PRZcf-ntqry=><~]))'
        ].join('|');
        var x = new RegExp(pattern, 'g');
        return x.test(txt);
    }
};
/**
 * Get ANSI codes from text
 * @param {string} txt
 * @returns {string[]}
 */
var getAnsi = function (txt) {
    if (['undefined', null].indexOf(typeof txt) !== -1) {
        txt = '';
    }
    var pattern = [
        '[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:[a-zA-Z\\d]*(?:;[a-zA-Z\\d]*)*)?\\u0007)',
        '(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PRZcf-ntqry=><~]))'
    ].join('|');
    var x = new RegExp(pattern, 'g');
    return txt.match(x);
};
/**
 * Remove ANSI Text from string
 * @param {string} txt
 * @returns {string}
 */
var removeAnsi = function (txt) {
    if (txt === void 0) { txt = ''; }
    if (['undefined', null].indexOf(typeof txt) !== -1) {
        txt = '';
    }
    var x = '[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:[a-zA-Z\\d]*(?:;[a-zA-Z\\d]*)*)?\\u0007)|(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PRZcf-ntqry=><~]))';
    return txt.toString().replace(new RegExp(x, 'g'), '');
};
exports.hasAnsi = hasAnsi;
exports.getAnsi = getAnsi;
exports.removeAnsi = removeAnsi;
